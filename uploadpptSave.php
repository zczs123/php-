...
if(isset($_POST['token'])&&$_POST['token']==$_SESSION['token'])
{
    ...
    {
        echo "<h2>上传PPT有错误发生！</h2>";
        header("refresh:3;url=./uploadppt.php");
        die();
    }
    //判断文件类型是不是PPT
    $arr1=array("application/vnd.openxmlformats-officedocument.presentationml.presentation");
    ...
    $dst_name="./ppt/".uniqid().".".$ext;
    move_uploaded_file($tmp_name,$dst_name);
    //将表单数据提交到数据库
    $title=$_POST['title'];
    $intro=$_POST['intro'];
    $pptsrc=$dst_name;//将PPT路径保存到数据库
    $addate=time();
    //判断添加的图片封面是否合法
    if($_FILES['uploadfile']['error']!=0)
    {
    ...
    $imgsrc=$des_name;
    //判断记录是否添加成功
    $sql="INSERT INTO ppt VALUES(null,'$title','$pptsrc','$intro',0,$addate,'$imgsrc')";
    if(mysqli_query($link,$sql))
    {
        echo "<h2>上传成功！</h2>";
        header("refresh:3;url=./ppt.php");
        die();
    }
}
    // else

...
if(isset($_POST['token'])&&$_POST['token']==$_SESSION['token'])
{
    ...
    {
        echo "<h2>上传视频有错误发生！</h2>";
        header("refresh:3;url=./uploadvideo.php");
        die();
    }
    //判断文件类型是不是视频
    $arr1=array("video/mp4");
    ...
    $dst_name="./video/".uniqid().".".$ext;
    move_uploaded_file($tmp_name,$dst_name);
    //将表单数据提交到数据库
    $title=$_POST['title'];
    $intro=$_POST['intro'];
    $videosrc=$dst_name;//将视频路径保存到数据库
    $addate=time();
    //判断添加的图片封面是否合法
    if($_FILES['uploadfile']['error']!=0)
    {
    ...
    $imgsrc=$des_name;
    //判断记录是否添加成功
    $sql="INSERT INTO video VALUES(null,'$title','$videosrc','$intro',0,$addate,'$imgsrc')";
